...
import { findPostByIdWithPrivacy, findCommentById, createCommentInTransaction, likeCommentInTransaction, unlikeCommentInTransaction, findCommentLikes, } from "./comment.repository.js";
export async function createComment(userId, userName, postId, input) {
    const post = await findPostByIdWithPrivacy(postId, userId);
    if (!post)
        throw new HttpError(404, "POST_NOT_FOUND", "Post was not found.");
    const comment = await createCommentInTransaction({ postId: post.id, authorId: userId, text: input.text }, userId);
    await notify({
...
export async function createReply(userId, userName, commentId, input) {
    const parent = await findCommentById(commentId);
    if (!parent || parent.parentId) {
        throw new HttpError(404, "COMMENT_NOT_FOUND", "Comment was not found.");
    }
    const reply = await createCommentInTransaction({ postId: parent.postId, parentId: parent.id, authorId: userId, text: input.text }, userId);
    await notify({
...
export async function likeComment(userId, userName, commentId) {
